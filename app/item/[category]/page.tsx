...
import { unslugify, formatPrice, slugify } from "@/lib/utils";
...
export default function ItemCategoryPage({
  ...
  const products = PRODUCTS.filter(
    (product) => slugify(product.category) === params.category,
  );

  return (
    <fieldset className="min-h-full w-full p-4 pt-16">
      <blockquote className="mx-auto max-w-2xl">
        <h2 className="text-center font-display text-2xl font-black capitalize text-yellow-900 lg:text-4xl">
      ...
      <ul className="mt-16 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {products.map((productItem, productItemIndex) => (
          <Link
            ...
            href={`/item/${params.category}/${slugify(productItem.name)}`}
